...
conn = pymysql.connect(host="127.0.0.1", user="root", password="leslie", db="article_spider", port=3306, charset='utf8')
cursor = conn.cursor()
...
def crawl_ips():
    ...
    for i in range(1, 3356):
        ...
        ip_list = []
        for tr in all_trs[1:]:
            speed_str = all_trs.xpath('td[7]/div/@title').extract()[0]
            if speed_str:
